...
function Line({ text, lineNo, active, T, fs, gutW, gutPr }: {
  text: string; lineNo: number; active: boolean;
  T: ReturnType<typeof getTokens>;
  fs: number; gutW: number; gutPr: number;
}) {
  ...
  return (
    <div style={{
      ...
      <span style={{
        width:`${gutW}px`, minWidth:`${gutW}px`, textAlign:"right", paddingRight:`${gutPr}px`,
        fontSize:`${fs - 1.5}px`, lineHeight:"22px", color: active ? T.cursorCol : T.lineNum,
        userSelect:"none", transition:"color 0.3s",
      }}>{lineNo}</span>
      <span style={{
        fontSize:`${fs}px`, lineHeight:"22px", whiteSpace:"pre",
...
export default function CodeAnimation() {
  ...
  // Responsive code sizing — smaller font + gutter on phones
  const isMobile  = useMediaQuery("(max-width:600px)");
  const fs        = isMobile ? 11   : 13.5;
  const gutW      = isMobile ? 30   : 40;
  const gutPr     = isMobile ? 10   : 18;
  ...
  return (
    <div style={{
      ...
      <div className="ttef-code" style={{
        ...
        {doneLines.map((ln, i) => (
          <Line key={i} text={ln} lineNo={i+1} active={false} T={T} fs={fs} gutW={gutW} gutPr={gutPr} />
        ))}
        ...
        {hasPartial && doneLines.length < LINES.length && (
          <div style={{
            ...
            <span style={{
              width:`${gutW}px`, minWidth:`${gutW}px`, textAlign:"right", paddingRight:`${gutPr}px`,
              fontSize:`${fs - 1.5}px`, lineHeight:"22px", color:T.cursorCol,
              userSelect:"none",
            }}>{activeLineNo}</span>
            <span style={{
              fontSize:`${fs}px`, lineHeight:"22px", whiteSpace:"pre",
